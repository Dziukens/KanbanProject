...
async function getWeatherByLocation(locationName, unitType) {
  const apiUrl = `${BASE_API_URL}?q=${locationName}&appid=${API_KEY}&units=${unitType}&lang=${LANGUAGE}`;

  return await fetchData(apiUrl);
}

async function getWeatherByPosition(lat, lon, unitType) {
  const apiUrl = `${BASE_API_URL}?lat=${lat}&lon=${lon}&appid=${API_KEY}&units=${unitType}&lang=${LANGUAGE}`;
